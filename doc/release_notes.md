...
### v0.5.1 (2025年7月12日) - Googleカレンダー認証エラー対応

#### 🔧 改善点

**🔐 認証エラー処理強化**
- **Googleカレンダー認証エラー自動検出**
  - GoogleCalendarServiceに認証エラー状態管理を追加
  - トークン無効化の自動検出機能
  - 認証エラー詳細情報の取得・保存

- **再認証UI機能**
  - 専用の再認証UIウィジェット（GoogleCalendarReauthWidget）
  - オレンジ色のアラートボックスで視覚的な警告表示
  - ワンクリック再認証機能
  - 再認証中のローディング表示
...
- **分析ダッシュボード統合**
  ...
  - 再認証成功後の自動データ再読み込み
  - 成功/失敗のフィードバック（SnackBar）

#### 🐛 バグ修正
- Googleカレンダー認証エラー時にユーザーに適切なフィードバックを提供
- 認証エラーが発生してもアプリケーションが正常に動作するよう改善
- カレンダーデータが表示されない問題の根本的解決

#### 🔧 技術的改善
- GoogleCalendarService.refreshToken()メソッドの戻り値をboolに変更
- 認証エラー状態の統一管理
- 再認証フローの最適化
- エラーハンドリングの強化
...
### v0.5.0 (2025年7月12日) - Phase 5 分析・通知機能統合完全実装

#### 🎉 新機能・改善点

**📊 分析機能強化（Step 1 - 100%完了）**
- **週間レポート機能**
  - サマリー、詳細分析、傾向の3タブ構成
...
#### 📝 備考
- Phase 5計画書（`doc/plans/phase5_analysis_notification_plan.md`）に基づく完全実装
- 本格運用開始準備完了
- 追加機能開発の基盤完成
- **Phase 5完全実装完了**: 分析・通知機能統合、統合テスト、品質向上を全て達成

### v0.4.3 (2025年7月12日) - 通知機能実装・lintエラー修正

#### 🎉 新機能・改善点
- **通知機能の完全実装**
  - `NotificationSettingsProvider`の完全実装
  - 習慣・タスク・AI通知のスケジューリング機能
  - テスト通知機能（習慣・タスク・AI通知のテスト送信）
  - 通知権限管理と統計機能
  - 最適化提案機能

- **lintエラーの大幅削減**
  - ビルドエラー0件達成
  - lintエラーを231件→188件に削減
  - `withOpacity`のdeprecated警告を`withValues`に修正
  - 不要な`toList()`の削除
  - 未使用インポートの削除

#### 🛠 技術的改善
- **文字化けと構文エラーの修正**
  - `notification_settings_provider.dart`の文字エンコーディング問題解決
  - 未終了文字列リテラルエラーの解消
  - 正しい日本語文字列への修正
...
#### 📝 備考
- Production-ready状態を維持
- 通知機能の基盤が完成し、今後の機能拡張が可能

### v0.4.2 (2025年7月12日) - ダッシュボードUIローディング安定化・実機特有問題の解決

#### 🎉 新機能・改善点
- **ダッシュボード初回表示時のUIちらつき・不要な柄表示（実機のみ）を根本解決**
  - ローディング状態の厳密化（データ取得完了まで本体UIを描画しない）
  - 実機でのみ発生していた「カードが重複・ループして見える」現象を完全解消
  - エミュレーターと実機の挙動差を吸収し、安定したUXを実現
- **UX向上**
  - ローディング画面の背景色統一
  - アニメーション・描画タイミングの最適化
...
#### 📝 備考
- 詳細な技術的背景・解決策はdoc/develop_trouble.md参照
